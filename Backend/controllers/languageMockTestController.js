const languageMockTestModel = require('../models/languageMockTestModel');
...
const getMocktestNumber = async (req, res) => {
    try {
        ...
        const lastMockTest = await languageMockTestModel
            .findOne({ languageName: languageName })
            .sort({ testNumber: -1 })
            .select('testNumber');
...
const createQuestionController = async (req, res) => {
    try {
        ...
        const newMockTest = await new languageMockTestModel({
            languageName,
            testNumber,
            testName,
            questions
...
const getQuestionController = async (req, res) => {
    try {
        const questions = await languageMockTestModel.find({});
        res.status(200).send({
            success: true,
...
const getLanguageQuestionController = async (req, res) => {
    try {
        const { lang } = req.params;
        const questions = await languageMockTestModel.find({ languageName: lang });
...
const getSingleQuestionController = async (req, res) => {
    try {
        const mocktest = await languageMockTestModel.findOne({ _id: req.params.mocktest });
...
const updateQuestionController = async (req, res) => {
    try {
        ...
        const mocktestfind = await languageMockTestModel.findOne({ _id: req.params.mocktest });
        ...
        // Update question
        const updatequestion = await languageMockTestModel.findByIdAndUpdate(
            mocktest,
            {
                languageName,
                testNumber,
...
const deleteQuestionController = async (req, res) => {
    try {
        await languageMockTestModel.findByIdAndDelete(req.params.mocktest);
        res.status(200).send({
            success: true,
